...
//    echo $wpdb->last_query;
    ...
    $pagination = array(
      'base' => add_query_arg( array('fpage'=> '%#%', 'num' => $pag_num, 'filter-user' => $owner, 'filter-type' => $media_type_filter, 'filter-date'=>$time_filter ) ), // http://example.com/all_posts.php%_% : %_% is replaced by format (below)
      'format' => '', // ?page=%#% : %#% is replaced by the page number
      'total' => ceil( ($cnt) / $pag_num),
      'current' => $fpage,
      'prev_next' => true,
...
//    var_dump($media_user_name);
    ...
    $kaltura_data = get_data_from_kaltura($time_filter);//data fetched from kaltura
...
function get_data_from_kaltura($time_filter) {
    global $bp,$kaltura_validation_data;
    ob_clean();
    ...
    switch ($time_filter){
        case  'last-week' :
             $timesince = getDate();
             $timesince = $timesince[0]-2*24*60*60;
             break;

        case  'last-month' :
            $timesince = getDate();
            $timesince = $timesince[0]-30*24*60*60;
            break;
        default :
            $timesince = 21600 ;
            break;
...
//    var_dump($pageIndex);
    ...
    $filter = new KalturaMediaEntryFilter();
    $pager  = new KalturaFilterPager();
    $filter->createdAtGreaterThanOrEqual = $timesince;
...
//    $_REQUEST['filter-date'], $_REQUEST['pick-date'], $_REQUEST['filter-user'], $_REQUEST['filter-type']
//    $where_date = false;
//    if(isset ($_REQUEST['pick-date']) && $_REQUEST['pick-date'] != ''){
//        $where_date = $_REQUEST['pick-date'];
//    }else{
//        switch ($_REQUEST['filter-date']) {
//            case -1:
//                $where_date = false;
//                break;
//            case 'last-week':
//                $where_date = 'md.date=timestamp';
//                break;
//            case 'last-month':
//                $where_date = 'md.date=timestamp';
//                break;
...
//
//    $where_user = '';
//    if(isset ($_REQUEST['filter-user']) && $_REQUEST['filter-user'] != ''){
//        if($where_date)
//            $where_user = "AND md.column1={$_REQUEST['filter-user']}";
//        else
//            $where_user = "md.column1={$_REQUEST['filter-user']}";
//    }
//
//    $where_type = '';
//    if(isset ($_REQUEST['filter-type']) && $_REQUEST['filter-type'] != ''){
//        if($where_user)
//            $where_type = "AND md.column2={$_REQUEST['filter-type']}";
//        else
//            $where_type = "md.column2={$_REQUEST['filter-type']}";
//
//    }
//var_dump($where_date);
//var_dump($where_user);
//var_dump($where_type);
//$qry =  "SELECT * FROM {$bp->media->table_media_data} WHERE $where_date $where_user $where_type";
//var_dump($qry);

    //PREGATI HELP CODE ENDS HERE
