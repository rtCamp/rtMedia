...
function rt_media_importer_page() {
    ...
    for($i = 0; $i<count($imported_pic_ids);$i++){
        $tocheck .= $imported_pic_ids[$i].',';
   ...
   $tocheck =  substr_replace($tocheck ,"",-1);
    ...
    $album_query = "SELECT * FROM {$wpdb->base_prefix}bp_album WHERE id NOT IN ($tocheck)";
   
    $album_result = $wpdb->get_results($album_query);

//    print_r($album_result);
...
//                                 $album_result[$i]->description="No Description";
                    ...
                    foreach ($album_result as $album_media) {
                        if (!in_array($album_media->id, $imported_pic_ids)) {
                            ...
                            echo '<td class="column-title"><img height= "30" width = "45px"  src= "'.$album_media->pic_org_url .'"><p>'.rt_get_user_login_name($album_media->owner_id).'</p></a></td>';
