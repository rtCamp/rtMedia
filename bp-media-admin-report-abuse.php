...
function bp_media_abused_content(){
      ...
      for($i=0;$i<count($result);$i++){
          $test123 = $kaltura_validation_data['client']->baseEntry->getByIds($result[$i]->entry_id);
        echo '<tr id ="ig-'.$result[$i]->id.'">';
        echo "<th><img class=rt-abuse-list src='".$test123[0]->thumbnailUrl."'/></th>";
        echo "<td align=center>".$result[$i]->media_owner."</td>";
        echo "<td align=center>".$result[$i]->abuse_reporter."</td>";
        echo '<td align=center><a href ="'.$result[$i]->abusued_url.'">'.$test123[0]->name.'</a></td>';
