...
require ( BP_MEDIA_PLUGIN_DIR . '/bp-media-admin.php' );//for admin control and kaltura setting1
...
require ( BP_MEDIA_PLUGIN_DIR . '/bp_media_video_featured_widget.php' );//for showing featured video as widget
require ( BP_MEDIA_PLUGIN_DIR . '/bp_media_photo_featured_widget.php' );//for showing featured photo as widget
require ( BP_MEDIA_PLUGIN_DIR . '/bp_media_media_widget.php' );//for showing photo/audio/video as widget
...
function media_setup_globals() {
    ...
    if(get_site_option('bp_rt_kaltura_url')) {
        ...
        $kaltura_validation_data['ks']      = $ks;
            ...
            $media_msg = __("Unable to Connect Media Hosting server");
            $media_notice = '<div class="updated fade"><p><strong>'.$media_msg.'</strong></p></div>';
            $kaltura_validation_data = null;
        }
    }
    else {
...
//        add_action('admin_menu', create_function("", 'echo \''.$media_notice.'\';'));
    ...
    update_option('bp_rt_dashboard_size',12);
    update_option('bp_rt_media_size',12);
...
function media_add_admin_menu() {
    ...
    add_submenu_page( 'bp-general-settings', __( 'BuddyPress Media', 'buddypress'), '<span class="rt-buddypress-admin-media-head">' . __( 'BuddyPress Media', 'buddypress' ) . '&nbsp;&nbsp;&nbsp;</span>', 'manage_options', 'bp-media-admin-reassign', 'rt_media_admin_page_reassign' );

         $mypage = add_submenu_page('bp-general-settings', //$parent
...
//            __('Dashboard old','buddypress'),//$page_title
//            '<span class="rt-buddypress-admin-media">&middot; '.__('Dashboard old','buddypress'). '&nbsp;&nbsp;&nbsp;</span>',//$menu_title
//            'manage_options',//$access_level
//            'bp-media-admin',//$file
//            "rt_media_admin_page" );//$function
    ...
    add_submenu_page('bp-general-settings', //$parent
            __('Kaltura Setting','buddypress'),//$page_title
             '<span class="rt-buddypress-admin-media">&middot; '.__('Settings','buddypress'). '&nbsp;&nbsp;&nbsp;</span>',//$menu_title
            'manage_options',//$access_level
            'bp-media-setup',//$file
      ...
      //this will load only when the page "bp-media-admin-reassign" is loaded
//      add_action( "admin_print_scripts-$mypage", 'rt_myplugin_admin_head' );
      add_action('admin_print_styles-' . $mypage, 'rt_my_plugin_admin_styles');
...
/**
 * jQuery file for drag - drop - save sequence for featured media widget
 */
function rt_myplugin_admin_head() {

    $js_path = BP_MEDIA_PLUGIN_URL.'/themes/media/js/';
    wp_enqueue_script( 'rt-jq-ui', $js_path.'jquery-ui-1.7.1.custom.min.js');

}

function rt_my_plugin_admin_styles() {
    $css_path = BP_MEDIA_PLUGIN_URL.'/themes/media/css/';
    wp_enqueue_style('rt-css-ui', $css_path.'rt-css-ui.css');
...
function bp_media_header_nav_setup() {//done
    global $bp;
//    $rt_action_var = rt_ret_action_variable($bp->current_action);
...
//    $av = $bp->action_variables[0];
    /* PRASAD CODE */
    $ca = rt_ret_action_token($bp->current_action);
    $av = rt_ret_action_variable($bp->current_action);
    /* PRASAD CODE END */
...
//        wp_enqueue_script( 'jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js');
         ...
         wp_enqueue_script( 'bp-media-single', $js_path.'single.js','jquery');
 ...
 */
function bp_pictures_get_pictures_for_user( $user_id, $media_type,$view,$group_id,$album_id,$type,$page, $per_page ) { //kapil

    return BP_Media_Picture::get_pictures_for_user( $user_id,$media_type,$view,$group_id,$album_id,$type,$page, $per_page); //kapil
}
/**
...
//    var_dump($bp->action_variables[0],is_media_exists($bp->action_variables[0]),$bp->current_component == 'media');
//    if($bp->action_variables[0] && is_media_exists($bp->action_variables[0]) && $bp->current_component == 'media') {
/* PRASAD CODE */ /* PRASAD CODE */
    if(is_media_exists($bp->current_action)) {
//        $media_link = $bp->root_domain . '/' . $bp->media->slug . '/' . $bp->current_action . '/' .$bp->action_variables[0];
        /* PRASAD CODE */
        $media_link = $bp->root_domain . '/' . $bp->media->slug . '/' . $bp->current_action;
        /* PRASAD CODE END */
...
add_action( 'bp_setup_nav', 'media_setup_nav' );
/* PRASAD CODE END */
...
function bp_media_screen_settings_menu() {
    do_action( 'bp_media_screen_settings_menu' );

    bp_core_load_template( apply_filters( 'bp_media_screen_settings_menu', 'members/single/home' ) );
}
...
//    var_dump('item id is =0------------',$object, $item_id, $content,'-------------');
    global $bp;
//    $item_id = $bp->action_variables[0];
    $item_id = rt_ret_action_variable($bp->current_action);
//    echo '$item_id='.$item_id; // this prints item_id
//    var_dump('~~~'.$object.'~~~'); // This prints the obj as media
    if ( 'media' == $object ) {
...
//            echo '$bp->media->id='.$bp->media->id; echo '<br>';
//            echo 'rt_ret_action_variable($bp->current_action)=='.rt_ret_action_variable($bp->current_action);
            if ( $bp->media->id ) {
                $args['primary_id'] = rt_ret_action_variable($bp->current_action); /* PRASAD CODE */
            }

//            var_dump($args['primary_id']);
...
add_filter( 'bp_dtheme_ajax_querystring', 'bp_media_dtheme_ajax_querystring_activity_filter', 1, 7 );
...
function bp_media_post_update( $args = '' ) {
    ...
    $defaults = array(
            ...
            'media_id' => rt_ret_action_variable($bp->current_action)//$bp->action_variables[0]
    );
//    var_dump('defautls :-----',$defaults);
//    var_dump('args = -----------',$args);
...
//    var_dump('~~~',$bp->media->current_media,'~~~');
...
function bp_media_dtheme_ajax_querystring_group_filter( $query_string ) {
    ...
    if ( $bp->groups->slug == $bp->current_component && $bp->current_action == $bp->media->slug ) {
        ...
        switch ($bp->action_variables[0]) {  // $bp->action_variables[0]
            case 'mediaall' :
                $args['scope'] = 'mediaall';
                break;
            case 'video' :
...
//to remove messages nav from its original position :: ashish
// remove_action( 'bp_setup_nav', 'messages_setup_nav' );
