 ...
 */
function bp_pictures_get_pictures_for_user( $user_id, $media_type,$view ) {

    return BP_Media_Picture::get_pictures_for_user( $user_id,$media_type,$view);
}
/**
...
function bp_directory_media_search_form() {
	...
	<form action="" method="get" id="search-media-form">
		<label><input type="text" name="s" id="media_search" value="<?php if ( isset( $_GET['s'] ) ) { echo attribute_escape( $_GET['s'] ); } else { _e( 'Search anything...', 'buddypress-media' ); } ?>"  onfocus="if (this.value == '<?php _e( 'Search anything...', 'buddypress-media' ) ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( 'Search anything...', 'buddypress-media' ) ?>';}" /></label>
