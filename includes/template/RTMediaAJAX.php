...
class RTMediaAJAX {
	...
	public function __construct() {
		...
		add_action( 'bp_' . RTMEDIA_BP_COMPONENT_ID . '_parse_query', array( $this, 'handle_load_more_media' ) );
		add_action( 'bp_' . RTMEDIA_BP_COMPONENT_ID . '_parse_query', array( $this, 'handle_lightbox_request' ) );
	}

	/**
	 * Hooked to "bp_{RTMEDIA_BP_COMPONENT_ID}_setup_actions" action
	 *
	 * Check if current request is single media ajax request and load main template accordingly.
	 */
	function handle_lightbox_request() {
	 ...
	 */
	function handle_load_more_media() {
	...
	function create_album() {
		...
		if ( wp_verify_nonce( $nonce, 'rtmedia_create_album_nonce' ) && isset( $_POST['name'] ) && $_POST['name'] && is_rtmedia_album_enable() ) {
			...
			if ( $_POST['context'] == "group" ) {
				$rtMediaNav->refresh_counts( $_POST['context_id'], array( "context"    => $_POST['context'],
				                                                          'context_id' => $_POST['context_id']
				) );
			} else {
				$rtMediaNav->refresh_counts( get_current_user_id(), array( "context"      => "profile",
