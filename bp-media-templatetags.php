 ...
 * 
 */
    function BP_User_Media_Template( $user_id_filter, $page, $per_page, $max, $media_type,$view = 'multiple' ) {
        global $bp;
        if ( !$user_id )
        ...
        if ( !$this->pictures = wp_cache_get( 'bp_pictures_for_user_' . $user_id, 'bp' ) ) {
            $this->pictures = bp_pictures_get_pictures_for_user( $user_id_filter, $media_type,$view);
...
function bp_has_media( $args = '' ) {
    ...
    $pictures_template = new BP_User_Media_Template( $user_id,$page, $per_page, $max,$scope, $view );
