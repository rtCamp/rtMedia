...
class BP_User_Media_Template {
    ...
    var $pictures;
    var $picture;
 //where the hell this initialized?
    var $in_the_loop;
    var $pag_page;
    var $pag_num;
    var $pag_links;
     ...
     *
     */
    function BP_User_Media_Template($user_id_filter, $page, $per_page, $max, $media_type, $view, $group_id, $album_id, $type='all-media-data') { //kapil
        global $bp;

//        var_dump($user_id_filter, $page, $per_page, $max, $media_type, $view,$group_id,$album_id,$type);
...
//            $this->pictures = bp_pictures_get_pictures_for_user( $user_id_filter, $media_type,$view,$group_id,$album_id,$type,$page, $per_page);//kapil
            $this->pictures = bp_pictures_get_pictures_for_user($user_id_filter, $media_type, $view, $group_id, $album_id, $type, $this->pag_page, $this->pag_num); //kapil
...
//		var_dump($this->pictures);
//		echo "<br />*******************<br />";
//		var_dump($this->pag_num);
//		echo "<br />*******************<br />";
//		var_dump($this->total_picture_count);
//		echo "<br />*******************<br />";
//		var_dump($this->pag_page);
//		echo "<br />*******************<br />";
//		var_dump();
    ...
    function has_pictures() {
        //echo $this->picture_count.'<br>';
        if ($this->picture_count)  //$this->picture_count
            return true;
        else
            return false;
    }
...
function bp_has_media($args = '') {
    ...
    if (($view != 'single') && ($type == 'popular' || $type == 'recent' || $type == 'rating'))
        $view = 'widget';
    $media_size = get_option('bp_rt_media_size'); //refenecr is in bp_media_admin.php for dynamic listing og media items
    ...
    $defaults = array(
        ...
        'group_id' => false,
        'view' => $view,
        'album_id' => false, //album_id = 1 >> default album //kapil //changed to false by Nemi
        'type' => 'all-media-data'
    );
...
//    echo '<pre>';
//    print_r($pictures_template);
//    echo '</pre>';    //$pictures_template->has_pictures();
//    echo '==='.$pictures_template->has_pictures();
...
function is_media_exists($id) {
    ...
    global $wpdb,$bp;
    $id = explode('_', $id);
    $qry = "SELECT id FROM {$bp->media->table_media_data} WHERE id='$id[1]'";
...
function rt_ret_action_variable($rt_current_action = '') {
    global $wpdb,$bp;
    $rt_current_action = explode('_', $rt_current_action);

    return $rt_current_action[1];
}

function rt_ret_action_token($rt_action_token = '') {
    global $wpdb,$bp;

    return strtok($rt_action_token,'_');
}
...
/**
 * This function prints the value of owner name whatever is fethed.
 */
function rt_bp_picture_owner() {
    echo bp_get_picture_owner();
}
    ...
    if (is_user_logged_in ()) {
        ...
        switch ($r) {
            case 0:
                ...
                echo '<div class="jquerystar">
                    <input name="star' . $local_entry . '" type="radio" value ="1" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="2" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="3" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="4" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="5" class="star star-rating-readonly"/>
                    </div>';
                break;
            ...
            case 1:
                ...
                echo '<div class="jquerystar">
                    <input name="star' . $local_entry . '" type="radio" value ="1" class="star star-rating-readonly" checked="checked"/>
                    <input name="star' . $local_entry . '" type="radio" value ="2" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="3" class="star star-rating-readonly" />
                    <input name="star' . $local_entry . '" type="radio" value ="4" class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value ="5" class="star star-rating-readonly"/>
            ...
            case 2:
                ...
                echo '<div class="jquerystar">
                    <input name="star' . $local_entry . '" type="radio" value = 1 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 2 class="star star-rating-readonly" checked="checked"/>
                    <input name="star' . $local_entry . '" type="radio" value = 3 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 4 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 5 class="star star-rating-readonly"/>
                    </div>';
            ...
            case 3:
                ...
                echo '<div class="jquerystar">
                    <input name="star' . $local_entry . '" type="radio" value = 1 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 2 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 3 class="star star-rating-readonly" checked="checked"/>
                    <input name="star' . $local_entry . '" type="radio" value = 4 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 5 class="star star-rating-readonly"/>
                    </div>';
                break;
            case 4:
                ...
                echo '<div class="jquerystar">
                    ...
                    <input name="star' . $local_entry . '" type="radio" value = 2 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 3 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 4 class="star star-rating-readonly" checked="checked"/>
                    <input name="star' . $local_entry . '" type="radio" value = 5 class="star star-rating-readonly"/>
                    </div>';
                break;
            case 5:
                ...
                echo '<div class="jquerystar">
                    ...
                    <input name="star' . $local_entry . '" type="radio" value = 3 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 4 class="star star-rating-readonly"/>
                    <input name="star' . $local_entry . '" type="radio" value = 5 class="star star-rating-readonly" checked="checked"/>
                    </div>';
                break;
            default:
                echo '<div class = "jquerystar"> Be 1st to rate it </div>';
 ...
 * @return var
 */
function bp_get_picture_tags() {
    global $pictures_template;
    return apply_filters('bp_get_picture_title', $pictures_template->picture->tags);
}

/**
 * It returns the single owner name from the Kaltura Server
 * @global object $pictures_template
 * @return var
 */
function bp_get_picture_owner() {
    global $pictures_template;
//    var_dump($pictures_template);
    $media_uploader = rt_who_owns_this_media($pictures_template->picture->db_id);

    $media_uploader_name = get_the_author_meta('user_nicename', $media_uploader);
    $media_uploader_login_name = get_the_author_meta('user_login', $media_uploader);
    return apply_filters('bp_get_picture_title', '<a href="' . get_bloginfo('url') . '/members/' . $media_uploader_login_name . '/profile">' . $media_uploader_name . '</a>');
}
...
function bp_get_picture_view_link() {
    ...
    $url = site_url() . '/' . BP_MEDIA_SLUG . '/' . $pictures_template->media_slug . '_' . $pictures_template->picture->db_id;
...
function get_media_data() {
    ...
    $img_id = $single_pic_template->picture_id;

//    var_dump($img_id);
//    var_dump($single_pic_template->media_type);
    ...
    if ($single_pic_template->picture_id == -9999)
        ...
        switch ($single_pic_template->media_type) {
            case '1':
                return bp_rt_media_video_library($img_id);
                break;
            case '2':
                return '<center><div class="photo-container"><img id =' . $img_id . ' src=' . bp_picture_view() . '/></div></center>

                        <input id="current-media-id" type = "hidden" value = "' . rt_ret_action_variable($bp->current_action) . '"/>
 ...
 *
 */
function bp_rt_media_video_library($img_id) {
     ...
     */
?>
    <input id="current-media-id" type = "hidden" value = "<?php echo rt_ret_action_variable($bp->current_action) ?>"/>
    <input id="current-user-id" type = "hidden" value = "<?php echo $bp->loggedin_user->id ?>"/>
    <script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/swfobject/2.2/swfobject.js"></script>
    <div id="asdf1">
    <?php echo rt_get_object_markup($img_id); ?>
</div>
 ...
 * To call in editor
 */
function rt_get_object_markup($img_id) {
...
?>
    ...
    <object name="kaltura_player" id="kaltura_player" type="application/x-shockwave-flash"
            ...
            data="<?php echo $kaltura_url; ?>/index.php/kwidget/cache_st/1274050232/wid/_<?php echo $partner_id ?>/uiconf_id/48410/entry_id/<?php echo $img_id ?>">
        ...
        <param name="movie" value="<?php echo $kaltura_url; ?>/index.php/kwidget/cache_st/1274050232/wid/_<?php echo $partner_id ?>/uiconf_id/48410/entry_id/<?php echo $img_id ?>"/>
 ...
 * this function displayes the description from the kaltura server
 */
function bp_single_picture_tags() {
    echo bp_get_picture_tags();
}
...
//    if (!$bp->action_variables[0])
    if (!rt_ret_action_variable($bp->current_action))
        return flase;

//    $pic_id = $bp->action_variables[0];
    /* PRASAD CODE */
    $pic_id = rt_ret_action_variable($bp->current_action);
    /* PRASAD CODE END */
...
function adjacent_picture_link($format, $link, $previous = true) {
    ...
    if (!$picture->id)
        return;
    /* PRASAD CODE */
    $picture->id = rt_ret_action_token($bp->current_action).'_'.$picture->id;
    /* PRASAD CODE END */
...
function bp_media_creation_time() {
    global $pictures_template;
    $format = "F j, Y";
    echo date($format, $pictures_template->pictures[0]->date_uploaded);
}
...
function bp_get_media_user_avatar($args = '', $link = false) {
    ...
    extract($r, EXTR_SKIP);
    /* PRASAD CODE */
    $rt_med_id = rt_ret_action_variable($bp->current_action);
    /* PRASAD CODE END */
    return apply_filters('bp_get_media_user_avatar', bp_core_fetch_avatar(array('item_id' => rt_who_owns_this_media($rt_med_id), 'type' => $type, 'alt' => $alt, 'class' => $class, 'width' => $width, 'height' => $height)));
}
...
//    $user_id = $wpdb->get_var("SELECT user_id from {$bp->media->table_media_data} WHERE ID = {$media_id} ");
    /* PRASAD CODE */
    $rt_med_id = rt_ret_action_variable($bp->current_action);  //  It will give the media_id by exploding.
    $user_id = rt_who_owns_this_media($rt_med_id); // Will give user id to whome media belongs.
    /* PRASAD CODE END */
...
//    $current_tab = $bp->action_variables[0];
    /* PRASAD CODE */
    $current_tab = rt_ret_action_token($bp->current_action);
...
//                        $id = $bp->action_variables[0];
                        $id = rt_ret_action_variable($bp->current_action);
