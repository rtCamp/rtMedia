 ...
 * @return Media
 */
    function get_pictures_for_user( $user_id,$media_type,$view) {
        ...
        if ( !bp_is_home() ) {
            if(!$user_id) {
                ...
                $picture_data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->media->table_media_data} ORDER BY id DESC") );
                $rt_pictures = BP_Media_Picture::get_media_data_by_user($user_id,$media_type,$view);
            }
            else {
...
//        echo 'Total is '.count($picture_data);
     ...
     */
    function get_media_data_by_user($user_id,$media_type,$view) {
    ...
    //this function should give the data uploaded by this user only.
        ...
        $list_all_elements = BP_Media_Picture::get_media_data($media_type,$view); //data from kaltura user seperated

        $j=0;
        if($user_id == 0) {
            $current_user_data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->media->table_media_data}") );
        }
        else {
            $current_user_data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bp->media->table_media_data} WHERE user_id= $user_id") );
        }
    ...
    function get_media_data($media_type,$view) {
        ...
        elseif ($view == 'single') {
            ...
            $query = "SELECT * FROM {$bp->media->table_media_data} where id = $current_item";
