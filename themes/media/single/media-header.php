    ...
    <hr/>
        ...
        <input type ="hidden" id="current-url" value=""/>
            <?php if ( bp_single_pic_exist() && (bp_single_pic_check_owner() || is_site_admin())):?>
                      <?php if ((isMediaOwner(bp_single_media_id()) &&(is_user_logged_in()) || is_site_admin())): ?>
        <div id="user-title" style="cursor: pointer;"><h2 class="rt-mediatitle" title="Click Here to Edit Media Title"><p><?php bp_single_picture_title() ?></p></h2></div>
                              <?php else: ?>
                    <div id="user-title"><h2><?php bp_single_picture_title() ?></h2></div>
            <?php endif; ?>
                    <div id="item-meta">
                        <div class="rt_navigation">
                             <div class="rt-prev"><?php previous_picture_link('%link', 'Previous') ?></div>
                             <div class="rt-next"><?php next_picture_link('%link',' Next') ?></div>
                        </div>
        ...
        <?php if ((isMediaOwner(bp_single_media_id()) &&( is_user_logged_in()) || is_site_admin())): ?>
                ...
                <div class="confirm">Delete From <b>Kaltura</b> Server too ? <a id="yes" style="cursor: pointer">YES</a> or <a id="no" style="cursor: pointer">NO</a> or <a id="cancel" style="text-decoration: none;cursor: pointer">Cancel</a></div>
                    ...
                    <div id="user_pic-description"><h3><?php bp_single_picture_description() ?></h3></div>
                        <?php endif; ?>
<?php $rt_media_tags = bp_get_picture_tags(); if ($rt_media_tags != '') :?>
<div id="user_pic_tags">Tags : <small><?php echo $rt_media_tags;?></small></div>
<?php endif;?>
