                ...
                if (isset($usage_info[$this->api_key]->status) && $usage_info[$this->api_key]->status) {
                    if (isset($usage_info[$this->api_key]->remaining) && $usage_info[$this->api_key]->remaining > 0) {
                        ...
                        if (!in_array($_SERVER['HTTP_HOST'], $blacklist)) {
                            ...
                            add_filter('rtmedia_allowed_types', array($this, 'allowed_types'));
                        }
                    }
                }
            }
...
//                }
//
//                if (isset($_FILES['rtmedia_file'])) {
//                    $ext = end(explode(".", $_FILES['rtmedia_file']["name"]));
//                    if (in_array($_FILES['rtmedia_file']['type'], array('audio/mp3', 'video/mp4')) || in_array($ext, array('mp3', 'mp4'))) {
//                        return $class;
//                    }
//                }
...
//        add_submenu_page('bp-media-settings', __('BuddyPress Media Audio/Video Encoding Service', 'rtmedia'), __('Audio/Video Encoding', 'rtmedia'), 'manage_options', 'bp-media-encoding', array($this, 'encoding_page'));
//        global $submenu;
...
//        $rtmedia_admin->render_page('rtmedia-encoding');
//    }
//    public function encoding_settings() {
//        add_settings_section('rtm-encoding', __('Audio/Video Encoding Service', 'rtmedia'), array($this, 'encoding_service_intro'), 'rtmedia-encoding');
//    }
//    public function encoding_tab($tabs) {
//        $encoding_tab = array(
//            'href' => get_admin_url(add_query_arg(array('page' => 'rtmedia-encoding'), 'admin.php')),
////                    'name' => __('Audio/Video Encoding', 'rt'),
//            'name' => __('Encoding', 'rt'),
//            'slug' => 'rtmedia-encoding'
//        );
//
//        $reordered_tabs = NULL;
//        if (count($tabs) > 2) {
...
//        return $tabs;
//    }
//    public function admin_bar_menu($rtmedia_admin_nav) {
//// Encoding Service
//        $admin_nav = array(
//            'parent' => 'rtmedia-menu',
//            'id' => 'rtmedia-encoding',
...
//        if (count($rtmedia_admin_nav) > 2) {
//            foreach ($rtmedia_admin_nav as $key => $nav) {
//                if ($key == 3)
//                    $reordered_admin_nav[] = $admin_nav;
//                $reordered_admin_nav[] = $nav;
//            }
//            $rtmedia_admin_nav = $reordered_admin_nav;
//        } else {
//            $rtmedia_admin_nav[] = $admin_nav;
//        }
//        return $rtmedia_admin_nav;
//    }
