...
class RTMediaPrivacy {
	...
	function enable_buddypress_privacy( $flag, $method, $func_args ){
		...
		if ( '0' != isset( $option['privacy_enabled'] ) && $option['privacy_enabled'] ){
	...
	function content(){
		...
		<form method='post'>
				...
				<div class="section">
					...
					<div class="columns large-5">
						<?php foreach ( $rtmedia->privacy_settings['levels'] as $level => $data ) { ?>
							<label><input type='radio' value='<?php echo $level; ?>'
										  name='rtmedia-default-privacy' <?php echo ( $default_privacy == $level ) ? "checked" : ""; ?> /> <?php _e( $data ); ?>
							</label><br/>
						<?php } ?>
					</div>
				</div>
			...
			<div class="submit">
				<input type="submit" name="submit" value="<?php esc_attr_e( 'Save Changes', 'rtmedia' ); ?>" id="submit"
					   class="auto">
			</div>
		</form>
	<?php
