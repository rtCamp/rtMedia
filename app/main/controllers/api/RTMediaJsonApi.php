...
class RTMediaJsonApi {
	...
	function rtmedia_api_process_wp_forgot_password_request(){
		...
		if ( empty( $_POST['user_login'] ) ){
			echo $this->rtmedia_api_response_object( 'FALSE', $ec_email_missing, $msg_email_missing );
			exit;
		}
		...
		if ( function_exists( 'rtmedia_api_followers' ) ){
			...
			foreach ( $followers as $follower ) {
				...
				$profile_fields['follower'][] = array(
					'id' => $follower, 'name' => $follower_data['name'],
	...
	function rtmedia_api_process_rtmedia_gallery_request(){
		...
		foreach ( $media_list as $media ) {
			$data = array(
				'id' => $media->id, 'media_title' => $media->media_title, 'album_id' => $media->album_id, 'media_type' => $media->media_type, 'media_author' => $media->media_author, 'url' => get_rtmedia_permalink( $media->id ), 'cover' => rtmedia_image( 'rt_media_thumbnail', $media->media_id, false )
			);
