...
class RTMediaUploadFile {
	 ...
	 * @var bool
	 */
	public $fake = false; // todo:find usage.
	...
	public function exif( $file ) {
		...
		if ( in_array( strtolower( $file_parts['extension'] ), array( 'jpg', 'jpeg', 'tiff' ), true ) ) {

			if ( function_exists( 'read_exif_data' ) ) {
				$exif = @read_exif_data( $file['file'] ); // phpcs:ignore Generic.PHP.DeprecatedFunctions.Deprecated, WordPress.PHP.NoSilencedErrors.Discouraged, PHPCompatibility.FunctionUse.RemovedFunctions.read_exif_dataDeprecated
			} else {
				// read_exif_data is deprecated since PHP 7.2.
				$exif = @exif_read_data( $file['file'] ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			}
