...
class RTMediaNav {
	...
	public function sub_nav() {
		...
		echo wp_kses( apply_filters( 'rtmedia_sub_nav_all', '<li id="rtmedia-nav-item-all-li" class="' . esc_attr( $all ) . '"><a id="rtmedia-nav-item-all" href="' . esc_url( trailingslashit( $link ) ) . RTMEDIA_MEDIA_SLUG . '/">' . esc_html__( 'All', 'buddypress-media' ) . '<span>' . esc_html( ( isset( $counts['total']['all'] ) ) ? rtmedia_number_to_human_readable( $counts['total']['all'] ) : 0 ) . '</span></a></li>' ), RTMedia::expanded_allowed_tags() );
		...
		if ( ( array_key_exists( 'media_type', $rtmedia_query->query ) && isset( $rtmedia_query->query['media_type'] ) && ( 'album' === $rtmedia_query->query['media_type'] ) ) || ( array_key_exists( 'media_type', $rtmedia_query->action_query ) && isset( $rtmedia_query->action_query->media_type ) && ( 'album' === $rtmedia_query->action_query->media_type ) ) ) {
		...
		if ( is_rtmedia_album_enable() ) {
			...
			$album_label              = esc_html( defined( 'RTMEDIA_ALBUM_PLURAL_LABEL' ) ? constant( 'RTMEDIA_ALBUM_PLURAL_LABEL' ) : __( 'Albums', 'buddypress-media' ) );
			echo wp_kses( apply_filters( 'rtmedia_sub_nav_albums', '<li id="rtmedia-nav-item-albums-li" class="' . esc_attr( $albums ) . '"><a id="rtmedia-nav-item-albums" href="' . esc_url( trailingslashit( $link ) ) . RTMEDIA_MEDIA_SLUG . '/album/">' . esc_html( $album_label ) . '<span>' . esc_html( ( isset( $counts['total']['album'] ) ) ? rtmedia_number_to_human_readable( $counts['total']['album'] ) : 0 ) . '</span></a></li>' ), RTMedia::expanded_allowed_tags() );
		}
		...
		foreach ( $rtmedia->allowed_types as $type ) {
			...
			$type_label = esc_html__( defined( 'RTMEDIA_' . $name . '_PLURAL_LABEL' ) ? constant( 'RTMEDIA_' . $name . '_PLURAL_LABEL' ) : $type['plural_label'], 'buddypress-media' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			echo wp_kses(
				apply_filters(
					'rtmedia_sub_nav_' . $type['name'],
					'<li id="rtmedia-nav-item-' . esc_attr( $type['name'] )
					. '-' . esc_attr( $context ) . '-' . esc_attr( $context_id ) . '-li" ' . $selected
					. '><a id="rtmedia-nav-item-' . esc_attr( $type['name'] ) . '" href="' . esc_url( $profile_link . RTMEDIA_MEDIA_SLUG . '/' . constant( 'RTMEDIA_' . $name . '_SLUG' ) . '/' ) . '">'
					. $type_label . '<span>' . esc_html( ( isset( $counts['total'][ $type['name'] ] ) ) ? rtmedia_number_to_human_readable( $counts['total'][ $type['name'] ] ) : 0 ) . '</span></a></li>',
					$type['name']
				),
				RTMedia::expanded_allowed_tags()
			);
		}
