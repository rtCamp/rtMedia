...
class RTMediaTemplate {
	...
	function check_return_upload(){
		global $rt_media_query;
		 if ( $rt_media_query->action_query->action != 'upload' ) return;
	...
	function check_return_edit() {
		global $rt_media_query;
		if ( $rt_media_query->action_query->action == 'edit' && count( $_POST ) )
			$this->save_edit();
		return $this->get_default_template();
	}
...
//                            print_r($_POST);die;
	...
	function check_return_delete() {
		...
		global $rt_media_query;
		if ( $rt_media_query->action_query->action != 'delete' )
			return;
		if ( ! count( $_POST ) )
			return;

		if ( isset( $rt_media_query->action_query->default ) && $rt_media_query->action_query->default == 'delete' ) {
			$this->bulk_delete();
		} else {
			if ( is_rt_media_single() ) {
				$this->single_delete();
	...
	function bulk_delete() {
		$nonce = $_POST[ 'rt_media_bulk_delete_nonce' ];
	...
	function check_return_merge() {
		global $rt_media_query;
		if ( $rt_media_query->action_query->action != 'merge' )
			return;
...
//                    print_r($album_contents); die;
	...
	function check_return_comments(){
		...
		if ( $rt_media_query->action_query->action != 'comment' ) return;


				if ( isset( $rt_media_query->action_query->id ) && count( $_POST ) ) {
					/**
					 * /media/comments [POST]
