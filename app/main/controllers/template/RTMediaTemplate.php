...
class RTMediaTemplate {
	...
	public function single_delete() {
		...
		if ( wp_verify_nonce( $nonce, 'rtmedia_' . $rtmedia_query->media[0]->id ) ) {
			...
			if ( ( function_exists( 'bp_get_group_url' ) || function_exists( 'bp_get_group_permalink' ) ) && isset( $media_obj[0] ) && isset( $media_obj[0]->context ) && 'group' === $media_obj[0]->context ) {
				$group       = groups_get_group( array( 'group_id' => $media_obj[0]->context_id ) );

				if ( isset( $bp->version ) && version_compare( $bp->version, '12.0.0', 'ge' ) ) {
					$parent_link = bp_get_group_url( $group );
				} else {
