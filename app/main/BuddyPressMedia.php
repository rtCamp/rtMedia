...
class BuddyPressMedia {
	...
	/**
	 * @var string $text_domain Used for defining BP_MEDIA_TXT_DOMAIN
	 */
	...
	function init() {
		...
		if ( file_exists( BP_MEDIA_PATH . '/languages/'
				. get_locale() . '.mo' ) )
			load_textdomain( BP_MEDIA_TXT_DOMAIN, BP_MEDIA_PATH . '/languages/'
					. get_locale() . '.mo' );
	...
	/**
	 *
	 * @param array $links default links created by WordPress on the Plugin's Admin Screen
	 * @param string $plugin_slug Plugin Slug, passed by WordPress
	 * @return array A new array of links that also includes a Settings link
	 */
	function settings_link( $links, $plugin_slug ) {
		/* create link */
		$plugin_name = plugin_basename( __FILE__ );

		$admin_link = $this->get_admin_url(
				add_query_arg(
						array('page' => 'bp-media-settings' ),
						'admin.php'
						)
				);
		...
		if ( $plugin_slug == $plugin_name ) {
			array_unshift(
					$links, sprintf( '<a href="%s">%s</a>', $admin_link,
	...
	function excerpt_lengths() {
		...
		$bp_media_default_excerpts = apply_filters( 'bpm_excerpt_lengths',
				$def_excerpt );
	}
	...
	public function admin_notice() {
		...
		if ( isset( $_GET[ 'bp_media_nag_ignore' ] ) && '0'
				== $_GET[ 'bp_media_nag_ignore' ] ) {
		...
		if ( ! get_user_meta( $user_id, 'bp_media_ignore_notice' ) ) {
			if ( defined( 'BP_VERSION' ) ) {
				if ( version_compare( BP_VERSION, BP_MEDIA_REQUIRED_BP, '<' ) ) {
					...
					printf( __( 'The BuddyPress version installed is an older
						version and is not supported, please update BuddyPress
						to use BuddyPress Media Plugin.<a class="alignright"
						href="%1$s">X</a>', $this->text_domain ),
							'?bp_media_nag_ignore=0' );
					echo "</p></div>";
				}
			} else {
				echo '<div class="error"><p>';
				printf( __( 'You have not installed BuddyPress. Please install
					latest version of BuddyPress to use BuddyPress Media plugin.
					<a class="alignright" href="%1$s">X</a>',
						$this->text_domain ), '?bp_media_nag_ignore=0' );
				echo "</p></div>";
			}
		}
	}
	...
	public function activate() {
		$bpmquery = new WP_Query(
				array( 'post_type' => 'bp_media', 'posts_per_page' => 1 )
				);
