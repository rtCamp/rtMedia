...
//			add_action( 'admin_init', array( $this, 'presstrends_plugin' ) );
		...
		public function rtmedia_permalink_notice() {
		    ...
		    <p> <b>rtMedia:</b> ' . esc_html__( ' You must', 'buddypress-media' ) . ' <a href="' . esc_url(admin_url( 'options-permalink.php' )) . '">' . esc_html__( 'update permalink structure', 'buddypress-media' ) . '</a> ' . esc_html__( 'to something other than the default for it to work.', 'buddypress-media' ) . ' </p>
		    </div>';
		}
		...
		public function rtmedia_addon_update_notice() {
			...
			?>
				<div class="error rtmedia-addon-upate-notice">
					<p>
						<strong><?php esc_html_e( 'rtMedia:', 'buddypress-media' ) ?></strong> <?php esc_html_e( 'Please update all premium add-ons that you have purchased from rtCamp from', 'buddypress-media' ) ?> <a href="https://rtcamp.com/my-account/" target="_blank"><?php esc_html_e( 'your account', 'buddypress-media' ) ?></a>. <a href="#" onclick="rtmedia_hide_addon_update_notice()" style="float:right"><?php esc_html_e( 'Dismiss', 'buddypress-media' ) ?></a>
					</p>
				</div>
				<script type="text/javascript">
		...
		public function rtmedia_advertisement() {
			...
			<div class='rtmedia-admin-ad'>
				<a href='http://rtcamp.com/products/rtmedia-pro/' target='_blank' title='rtMedia Pro'>
					<img src='<?php echo esc_url($src); ?>' alt="<?php esc_html_e( 'rtMedia Pro is released', 'buddypress-media' ); ?>"/>
				</a>
			</div>
			<?php
		}
		...
		public function rtmedia_dashboard_widget_function() {
							...
							if ( $results ) {
								foreach ( $results as $media ) {
									if ( defined( strtoupper( 'RTMEDIA_' . $media->media_type . '_PLURAL_LABEL' ) ) ) {
										?>
										<tr>
											<td class="b"> <?php echo absint($media->count); ?> </td>
			...
			<div class="rtm-meta-container">
				<ul class="rtm-meta-links">
					...
					<li><a href="https://rtcamp.com/rtmedia/"><?php esc_html_e( 'Homepage', 'buddypress-media' ); ?></a></li>
					<li><a href="admin.php?page=rtmedia-support#rtmedia-general"><?php esc_html_e( 'Free Support', 'buddypress-media' ); ?></a></li>
					<li><a href="https://rtcamp.com/products/category/rtmedia/"><?php esc_html_e( 'Premium Addons', 'buddypress-media' ); ?></a></li>
				</ul>
			</div>
			<?php
		}
		...
		public function plugin_add_settings_link( $links ) {
			$settings_link = '<a href="' . esc_url(admin_url( 'admin.php?page=rtmedia-settings' )) . '">Settings</a>';
			array_push( $links, $settings_link );
			$settings_link = '<a href="' . esc_url(admin_url( 'admin.php?page=rtmedia-support' )) . '">Support</a>';
			array_push( $links, $settings_link );
		...
		public function add_reencode_link( $actions, $post ) {
			...
			if ( is_array( $mime_type_array ) && '' != $mime_type_array && 'video' == $mime_type_array[ 0 ] ) {
				$actions[ 'reencode' ] = '<a class="submitdelete" onclick="return rtmedia_regenerate_thumbs(' . $post->ID . ')" href="#">' . esc_html__( 'Regenerate Thumbnail', 'buddypress-media' ) . '</a>';
			}
		...
		public function admin_bar_menu( $admin_bar ) {
			...
			$admin_bar->add_menu( array(
				'id' => 'rtMedia',
				'title' => 'rtMedia',
				'href' => esc_url(admin_url( 'admin.php?page=rtmedia-settings' )),
				'meta' => array(
			...
			$admin_bar->add_menu( array(
				...
				'href' => esc_url(admin_url( 'admin.php?page=rtmedia-addons' )),
				'meta' => array(
					'title' => esc_html__( 'Addons', 'buddypress-media' ),
					'target' => '_self',
				),
			...
			$admin_bar->add_menu( array(
				...
				'href' => esc_url(admin_url( 'admin.php?page=rtmedia-support' )),
				'meta' => array(
					'title' => esc_html__( 'Support', 'buddypress-media' ),
					'target' => '_self',
				),
			...
			$admin_bar->add_menu( array(
				...
				'href' => esc_url(admin_url( 'admin.php?page=rtmedia-themes' )),
				'meta' => array(
					'title' => esc_html__( 'Themes', 'buddypress-media' ),
					'target' => '_self',
				),
			...
			$admin_bar->add_menu( array(
				...
				'href' => esc_url(admin_url( 'admin.php?page=rtmedia-hire-us' )),
				'meta' => array(
					'title' => esc_html__( 'Hire Us', 'buddypress-media' ),
					'target' => '_self',
				),
			...
			if ( has_filter( 'rtmedia_license_tabs' ) || has_action( 'rtmedia_addon_license_details' ) ) {
				$admin_bar->add_menu( array(
					...
					'href' => esc_url(admin_url( 'admin.php?page=rtmedia-license' )),
					'meta' => array(
		...
		public function ui( $hook ) {
			...
			if ( in_array( $hook, $admin_pages ) || strpos( $hook, 'rtmedia-migration' ) ) {
				...
				$rtmedia_admin_strings = array(
					...
					'something_went_wrong' => esc_html__( 'Something went wrong. Please <a href onclick="location.reload();">refresh</a> page.', 'buddypress-media' ),
		...
		public function menu() {
			...
			add_submenu_page( 'rtmedia-settings', esc_html__( 'Settings', 'buddypress-media' ), esc_html__( 'Settings', 'buddypress-media' ), 'manage_options', 'rtmedia-settings', array( $this, 'settings_page' ) );
			add_submenu_page( 'rtmedia-settings', esc_html__( 'Addons', 'buddypress-media' ), esc_html__( 'Addons', 'buddypress-media' ), 'manage_options', 'rtmedia-addons', array( $this, 'addons_page' ) );
			add_submenu_page( 'rtmedia-settings', esc_html__( 'Support', 'buddypress-media' ), esc_html__( 'Support', 'buddypress-media' ), 'manage_options', 'rtmedia-support', array( $this, 'support_page' ) );
			add_submenu_page( 'rtmedia-settings', esc_html__( 'Themes', 'buddypress-media' ), esc_html__( 'Themes', 'buddypress-media' ), 'manage_options', 'rtmedia-themes', array( $this, 'theme_page' ) );
			add_submenu_page( 'rtmedia-settings', esc_html__( 'Hire Us', 'buddypress-media' ), esc_html__( 'Hire Us', 'buddypress-media' ), 'manage_options', 'rtmedia-hire-us', array( $this, 'hire_us_page' ) );
			if ( has_filter( 'rtmedia_license_tabs' ) || has_action( 'rtmedia_addon_license_details' ) ) {
				add_submenu_page( 'rtmedia-settings', esc_html__( 'Licenses', 'buddypress-media' ), esc_html__( 'Licenses', 'buddypress-media' ), 'manage_options', 'rtmedia-license', array( $this, 'license_page' ) );
			}

			$obj_encoding = new RTMediaEncoding( true );

			if ( $obj_encoding->api_key ) {
				add_submenu_page( 'rtmedia-settings', esc_html__( 'Regenerate Thumbnail', 'buddypress-media' ), esc_html__( 'Regen. Thumbnail ', 'buddypress-media' ), 'manage_options', 'rtmedia-regenerate', array( $this, 'rt_regenerate_thumbnail' ) );
			}
		}
		...
		public function rt_regenerate_thumbnail() {
			...
			?>
			<div class="wrap">
				<h2> rtMedia: <?php esc_html_e( 'Regenerate Video Thumbnails', 'buddypress-media' ); ?> </h2>
				<?php
				...
				<div id="rt-migration-progress">
					...
					?>
					<p> <?php esc_html_e( 'Total Videos', 'buddypress-media' ) ?> : <span class='rt-total'><?php echo esc_html($total); ?></span>
					</p>

					<p> <?php esc_html_e( 'Sent of regenerate thumbails', 'buddypress-media' ) ?> : <span class='rt-done'>0</span></p>

					<p> <?php esc_html_e( 'Fail to regenerate thumbails', 'buddypress-media' ) ?> : <span class='rt-fail'>0</span></p>
				...
				<script>
					...
					var db_done = 0;
					var db_fail = 0;
					var db_total = <?php echo esc_html($total); ?>;
					var indx = 0;
				...
				if ( ! $site_option || 'hide' != $site_option ) {
					...
					if ( isset( $videos_without_thumbs ) && is_array( $videos_without_thumbs ) && sizeof( $videos_without_thumbs ) > 0 ) {
						echo '<div class="error rtmedia-regenerate-video-thumb-error">
								<p>
								' . sprintf( kses ( __( "You have %s videos without thumbnails. Click <a href='%s'> here </a> to generate thumbnails. <a href='#' onclick='rtmedia_hide_video_thumb_notice()' style='float:right'>Hide</a>", 'buddypress-media' )), sizeof( $videos_without_thumbs ), admin_url( 'admin.php?page=rtmedia-regenerate' ) ) . '
								</p>
								</div>';
						?>
						<script type="text/javascript">
				...
				<div>
					<h2 class="nav-tab-wrapper"><?php $this->rtmedia_tabs(); ?>
						<span class="alignright by">
							<a class="rt-link" href="http://rtcamp.com/?utm_source=dashboard&utm_medium=plugin&utm_campaign=buddypress-media" target="_blank" title="rtCamp : <?php esc_html_e( 'Empowering The Web With WordPress', 'buddypress-media' ); ?>">
								...
								<div class="bp-media-metabox-holder">
									<div class="rtm-button-container top">
										<?php if ( isset( $_GET[ 'settings-saved' ] ) && $_GET[ 'settings-saved' ] ) { ?>
											<div class="rtm-success rtm-fly-warning rtm-save-settings-msg"><?php esc_html_e( 'Settings saved successfully!', 'buddypress-media' ); ?></div>
										<?php } ?>
										<input type="hidden" name="rtmedia-options-save" value="true">
										<input type="submit" class="rtmedia-settings-submit button button-primary button-big" value="<?php esc_html_e( 'Save Settings', 'buddypress-media' ); ?>">
									</div>
									<?php
		...
		public function admin_sidebar() {
			...
			$addons = '<div id="social" class="rtm-social-share">
											<p><a href="http://twitter.com/home/?status=' . $message . '" class="button twitter" target= "_blank" title="' . esc_html__( 'Post to Twitter Now', 'buddypress-media' ) . '">' . esc_html__( 'Post to Twitter', 'buddypress-media' ) . '<span class="dashicons dashicons-twitter"></span></a></p>
											<p><a href="https://www.facebook.com/sharer/sharer.php?u=http://rtcamp.com/rtmedia/" class="button facebook" target="_blank" title="' . esc_html__( 'Share on Facebook Now', 'buddypress-media' ) . '">' . esc_html__( 'Share on Facebook', 'buddypress-media' ) . '<span class="dashicons dashicons-facebook"></span></a></p>
											<p><a href="http://wordpress.org/support/view/plugin-reviews/buddypress-media?rate=5#postform" class="button wordpress" target= "_blank" title="' . esc_html__( 'Rate rtMedia on Wordpress.org', 'buddypress-media' ) . '">' . esc_html__( 'Rate on Wordpress.org', 'buddypress-media' ) . '<span class="dashicons dashicons-wordpress"></span></a></p>
											<p><a href="' . sprintf( '%s', 'https://rtcamp.com/feed/' ) . '" class="button rss" target="_blank" title="' . esc_html__( 'Subscribe to our Feeds', 'buddypress-media' ) . '">' . esc_html__( 'Subscribe to our Feeds', 'buddypress-media' ) . '<span class="dashicons dashicons-rss"></span></a></p>
								</div>';
			...
			$branding = '<form action="http://rtcamp.us1.list-manage1.com/subscribe/post?u=85b65c9c71e2ba3fab8cb1950&amp;id=9e8ded4470" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
                            <div class="mc-field-group">
									...
									<input type="submit" value="' . esc_html__( 'Subscribe', 'buddypress-media' ) . '" name="subscribe" id="mc-embedded-subscribe" class="button">
			...
			//new RTMediaAdminWidget ( 'latest-news', esc_html__( 'Latest News', 'buddypress-media' ), $news );
		...
		public function plugin_meta_premium_addon_link( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			if ( plugin_basename( RTMEDIA_PATH . 'index.php' ) == $plugin_file ) {
				$plugin_meta[] = '<a href="https://rtcamp.com/rtmedia/addons/?utm_source=dashboard&#038;utm_medium=plugin&#038;utm_campaign=buddypress-media" title="' . esc_html__( 'Premium Add-ons', 'buddypress-media' ) . '">' . esc_html__( 'Premium Add-ons', 'buddypress-media' ) . '</a>';
			}
		...
		public function upload_filetypes_error() {
			...
			if ( isset( $rtmedia->options[ 'audio_enabled' ] ) && $rtmedia->options[ 'audio_enabled' ] ) {
				if ( ! in_array( 'mp3', $upload_filetypes ) ) {
					echo '<div class="error upload-filetype-network-settings-error"><p>' . sprintf( esc_html__( 'You have audio enabled on BuddyPress Media but your network allowed filetypes do not permit uploading of mp3. Click <a href="%s">here</a> to change your settings manually.', 'buddypress-media' ), network_admin_url( 'settings.php#upload_filetypes' ) ) . '
                            <br /><strong>' . esc_html__( 'Recommended', 'buddypress-media' ) . ':</strong> <input type="button" class="button update-network-settings-upload-filetypes" class="button" value="' . esc_html__( 'Update Network Settings Automatically', 'buddypress-media' ) . '"> <img style="display:none;" src="' . admin_url( 'images/wpspin_light.gif' ) . '" />
                        </p>
                        </div>';
					$flag = true;
