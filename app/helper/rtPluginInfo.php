...
class rtPluginInfo {
    ...
    public static function fromJson($json, $triggerErrors = false) {
        ...
        if (empty($apiResponse) || !is_object($apiResponse)) {
            if ($triggerErrors) {
                trigger_error(
                        sprintf( __( "Failed to parse plugin metadata. Try validating your .json file with %s", 'buddypress-media' ), 'http://jsonlint.com/' ), E_USER_NOTICE
                );
            }
            return null;
        }
        ...
        if (!$valid) {
            if ($triggerErrors) {
                trigger_error(
                        __( "The plugin metadata file does not contain the required 'name' and/or 'version' keys.", 'buddypress-media'), E_USER_NOTICE
                );
            }
            return null;
        }
