...
//            $new_qs[] = 'user_id=' . $user_id;
    ...
    // code by ashish showing the top rated media -- starts here
    if($_BP_COOKIE['bp-' . $object . '-filter'] == 'recent-media-data') {
        $user_id = ( $bp->displayed_user->id ) ? $bp->displayed_user->id : $bp->loggedin_user->id;
        $new_qs[] = 'user_id=' . $user_id;
    }

    if($_BP_COOKIE['bp-' . $object . '-filter'] == 'commented-media-data') {
        $user_id = ( $bp->displayed_user->id ) ? $bp->displayed_user->id : $bp->loggedin_user->id;
        $new_qs[] = 'user_id=' . $user_id;
    }

    if($_BP_COOKIE['bp-' . $object . '-filter'] == 'top-rated-media-data') {
        $user_id = ( $bp->displayed_user->id ) ? $bp->displayed_user->id : $bp->loggedin_user->id;
        $new_qs[] = 'user_id=' . $user_id;
    }
    // ends here for showing the top rated/most commented media ..
...
//        var_dump($bp->current_component, $bp->current_action);

    if ((!( 'upload' == $_BP_COOKIE['bp-' . $object . '-scope'] ))&& ($bp->current_action != 'photo'))
          bp_init_media();
...
//        wp_enqueue_script('jquery-min','http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js');
        wp_enqueue_script('thickbox',$js_path.'thickbox.js','jquery');

//        var_dump($_POST);
        if(is_kaltura_configured()):

            if('photo'== $bp->current_action && 'media' == $bp->current_component ):
                  ?><script> tb_init('a.thickbox, area.thickbox, input.thickbox');</script><?php

                  bp_media_locate_template( array( "$object/photo/$object-loop.php" ), true );
             else:
...
function rt_media_upload() {
    ...
    echo $rt_entry_id_list       =  $_POST['rt_entry_id_list'];
